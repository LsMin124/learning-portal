...
CHAPTERS: dict[int, dict[str, int]] = {
    ...
    29: {"29.1": 910, "29.2": 913, "29.3": 933},
    30: {"30.1": 952, "30.2": 954},
    31: {"31.1": 970},  # alternate 다수 → 추출 후 QA
}
...
MANUAL: dict[int, dict[str, tuple[float, float, float, float]]] = {
    ...
    29: {  # union OFF
        "29.1": (104.0,  90.0, 480.0, 206.0),  # 페이지 상단 figure(하단 prose 'Figure 29.1.' 오탐 교정). 좌측 캡션 + takeover 분류 트리
        "29.3": (106.0, 391.0, 579.0, 700.0),  # 페이지 하단 figure. 좌측 캡션 + 연도별 막대 + 'Years'축 + 내부 note + SOURCE
    },
    30: {  # union OFF
        "30.1": (104.0,  90.0, 531.0, 440.0),  # 좌측 캡션 + A.stock-based/B.flow-based 2패널 insolvency + 내부 note, 본문(y565) 제외
    },
}
