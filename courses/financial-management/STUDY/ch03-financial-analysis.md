...
> **핵심 한 문장**: 재무제표의 *절대 숫자* 는 회사마다 규모가 달라 비교가 안 된다 — 3장은 그것을 **비율(scale-free)** 로 바꿔 견주고, ROE 를 세 조각으로 분해(DuPont)하며, 매출 예측에서 *필요 외부자금(EFN)* 과 *지속가능성장률(SGR)* 까지 끌어낸다.

2장이 재무제표를 *읽는* 법이라면, 3장은 그것으로 *판단하는* 법이다:

1. **비교 가능하게** (§1): Apple 의 순이익 $100B 와 스타트업 $1M 은 직접 못 견준다 → 각 항목을 총자산·매출로 나눈 *common-size* 로 규모를 지운다.
2. **다섯 렌즈** (§2): liquidity(단기 생존)·solvency(부채 감당)·efficiency(자산 활용)·profitability(수익성)·market(시장 평가) — 비율은 *카테고리별로* 봐야 의미가 산다.
3. **ROE 를 쪼갠다 — DuPont** (§3): $ROE = 마진 \times 자산회전율 \times 레버리지$. 같은 ROE 라도 *어디서 왔나*(마진형 vs 회전형 vs 부채형)가 전혀 다른 회사를 말해준다.
4. **미래를 계획한다** (§4–5): *percentage-of-sales* 로 매출 성장에 필요한 자산·자금을 추정 → *EFN*(외부에서 더 끌어와야 할 돈). 외부조달 없이 가능한 성장이 *SGR*, 내부유보만으로는 *IGR*.

한 문장으로: **재무분석은 "비율로 비교하고, ROE 를 분해하고, 성장에 드는 돈을 미리 계산하는" 작업이다.**
