...
| 전치 | `[ω]ᵀ = −[ω]` |
| 회전 변환 | `R [ω] Rᵀ = [Rω]` |
| 제곱 | $[\omega]^2 = \omega \omega^T - \lVert \omega \rVert^2 I$ |
...
| 케이스 | 정규화 | v |
|--|--|--|
| 회전 있음 (h finite) | $\lVert \omega \rVert = 1$ | `v = −ω̂ × q + h ω̂` |
| 순수 회전 (h=0) | $\lVert \omega \rVert = 1$ | `v = −ω̂ × q` |
| 순수 병진 | $\lVert v \rVert = 1$, `ω = 0` | (단위 병진 방향) |
